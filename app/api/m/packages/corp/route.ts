...
function normalizeTagsForWrite(raw: any) {
  ...
  const tags = { ...raw }; const map: Record<string, any> = {};
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (validClientId) {
      const owned = await prisma.client.findFirst({ where: { id: validClientId, hospitalId: hid }, select: { id: true } });
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const url = new URL(req.url);
    const clientId = (url.searchParams.get("clientId") || "").trim() || null;
    const r = await prisma.package.deleteMany({ where: { hospitalId: hid, category: "CORP", ...(clientId ? { clientId } : {}) } });
