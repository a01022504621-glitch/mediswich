...
function GridByClient({
  ...
  const optCount = (p: CorpPackage) =>
    p.groupOrder
      .filter((g) => g !== "base")
      .reduce((a, g) => a + Math.max(0, p.groupMeta[g]?.chooseCount ?? 0), 0);
    ...
    <section className={card}>
      ...
      <div className={body}>
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <button className={clsx(btnPrimary, busy && "opacity-50")} onClick={onAdd} disabled={busy}>
            패키지 추가
          </button>
        </div>
                  ...
                  {list.map((p) => {
                    ...
                    const billing =
                      p.billing?.type === "subscription"
                        ? `구독 ${p.billing?.price?.toLocaleString() ?? "-"}원/${p.billing?.period ?? "monthly"}`
                        : "일시결제";
                    return (
                      <div key={p.id} className="flex flex-col gap-3 rounded-xl border p-4 shadow-sm">
                        ...
                        <div className="flex items-center justify-between border-t pt-2">
                          ...
                          <div className="flex gap-2">
                            <button className={btn} onClick={() => onEdit(p.id)} disabled={busy}>
                              수정
                            </button>
                            <button className={btn} onClick={() => onClone(p.id)} disabled={busy}>
                              복제
                            </button>
                            <button
...
export default function CorpPackagesPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!alive) return;
      setPackages(list as CorpPackage[]);
      savePackages(NS, list);
      // 코드맵 로드(네임스페이스 분리)
      setCodeMap(loadCodes(NS as any));
      // 엑셀 로드
      loadExcel()
        .then(setAllExams)
  ...
  const refreshFromDB = useCallback(async () => {
    const list = (await loadPackagesDBFirst(NS)) as CorpPackage[];
    setPackages(list);
    savePackages(NS, list);
  }, []);

  const saveList = (next: CorpPackage[]) => {
    setPackages(next);
  ...
  const setChooseCount = (gid: GroupId, n: number) =>
    setDraft((prev) => ({
      ...
      groupMeta: {
        ...prev.groupMeta,
        [gid]: { ...prev.groupMeta[gid], chooseCount: Math.max(0, Math.floor(n || 0)) },
      },
    }));
  ...
  const toggleExam = (examId: string) => {
    ...
    setDraft((prev) => {
      ...
      if (idx >= 0) {
        ...
        here.push({ examId, name: ex?.name || "", sex: "A", code: codeMap[examId] || "" });
  ...
  // 코드맵을 네임스페이스(CORP)로 분리하여 저장
  const setCode = (examId: string, code: string) => {
    // 1) 즉시 UI 반영
    setCodeMap((prev) => ({ ...prev, [examId]: code }));
    // 2) 비동기 저장(반환값 사용 안 함)
    void upsertCode(examId, code, NS as any);
    // 3) 드래프트 내 행도 동기화
    setDraft((prev) => {
      const cp = structuredClone(prev) as CorpPackage;
      cp.groupOrder.forEach(
        (gid) => (cp.groups[gid] = (cp.groups[gid] || []).map((r) => (r.examId === examId ? { ...r, code } : r)))
      );
      return cp;
    });
  };
  ...
  const safePublish = useCallback(
    async (next: CorpPackage[]) => {
      if (busy) return;
      setBusy(true);
      try {
        saveList(next);             // 로컬 저장(보조)
        await publishPackages(NS as any, next); // 서버 반영
        await refreshFromDB();      // DB 재조회로 일관성 확보
        saveCodes(codeMap, NS as any);
      } finally {
        setBusy(false);
      }
    },
    [busy, codeMap, refreshFromDB]
  );
      ...
      <div className="p-5 md:p-8 space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold tracking-tight">
            <span className="text-sky-600">기업검진</span> 패키지 등록
          </h1>
        </div>
  ...
  /* 편집 */
  // ⬇️ SWC 파싱 오류 회피: 타입 단언 대신 변수 타입 지정
  const billing: Billing = draft.billing ?? { type: "one_time" };
    ...
    <div className="p-5 md:p-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            className={clsx(btnPrimary, (!isValid || busy) && "cursor-not-allowed opacity-50")}
            onClick={saveDraft}
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setDraft((prev) => ({
                  ...
                  billing:
                    type === "subscription"
                      ? { ...(prev.billing || {}), type, enabled: true }
                      : { type: "one_time" },
                }));
              }}
              disabled={busy}
                ...
                <input
                  ...
                  onChange={(e) =>
                    setDraft((prev) => ({
                      ...prev,
                      billing: {
                        ...(prev.billing || { type: "subscription", enabled: true }),
                        price: Number(e.target.value.replace(/[^0-9]/g, "")) || 0,
                      },
                    }))
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    setDraft((prev) => ({
                      ...
                      billing: {
                        ...(prev.billing || { type: "subscription", enabled: true }),
                        intervalCount: Math.max(1, Number(e.target.value || 0)),
                      },
                    }))
                  }
                  disabled={busy}
            ...
            <>
                ...
                <div className="inline-flex rounded-full bg-gray-100 p-1">
                  {draft.groupOrder.map((gid) => {
                    ...
                    return (
                      <button
                        key={gid}
                        className={clsx("rounded-full px-4 py-2 text-sm transition", color)}
              ...
              <div className="max-h-[60vh] overflow-y-auto px-6 pb-6">
                {(draft.groups[activeGroup] || []).length === 0 ? (
                  <div className="rounded-xl border py-12 text-center text-gray-400">
                  ...
                  <div className="space-y-2">
                    {(draft.groups[activeGroup] || []).map((row) => (
                      <div
                        ...
                        style={{
                          gridTemplateColumns: "minmax(220px,1fr) 88px minmax(220px,1fr) 120px 48px",
                        }}
                      >
                        ...
                        />
                        <button
                          className="px-2 text-xs text-gray-500 hover:text-gray-900"
            ...
            <div className="space-y-4 p-6">
              <div className="flex items-end gap-2">
                <div className="flex-1">
                  ...
                  <input className={input} value={addonName} onChange={(e) => setAddonName(e.target.value)} disabled={busy} />
                </div>
                <div>
                  <label className="mb-1 block text-xs font-semibold text-gray-600">성별</label>
                  <select className={inputSm} value={addonSex} onChange={(e) => setAddonSex(e.target.value as any)} disabled={busy}>
                ...
                <button className={btn} onClick={addAddon} disabled={busy}>
                  추가
                </button>
              </div>
                  ...
                  draft.addons.map((a, i) => (
                    <div key={i} className="grid grid-cols-12 items-center gap-0 border-t px-3 py-2 text-sm">
                      ...
                      <div className="col-span-1 text-right">
                        <button className="text-xs text-gray-500 hover:text-red-600" onClick={() => removeAddon(i)} disabled={busy}>
                          삭제
                        </button>
                      </div>
                    </div>
